...
var events = require('events'),
  ...
  users = require('../users'),
  PhantNotify = require('phant-notify-email'),
  notify = PhantNotify({host: config.phant.host});
...
app.log = function(req, res) {
  ...
  this.throttler.available(pub, function(ready, used, limit, reset) {
    ...
    input.validator.fields(id, data, function(err, valid) {
      ...
      // xleo
      notify.create(
        {to: users[data.mac].email},
        {title: 'Kudos from ' + users[data.mac].name + ': ' + data.button},
        function(err) {
